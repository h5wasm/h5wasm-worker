...
export class GroupProxy {
  ...
  async get(name: string = "/") {
    ...
    if (dumb_obj?.type === "Group") {
      const group_obj = dumb_obj as Group;
      const new_group_proxy = await new remote.Group(group_obj.file_id, group_obj.path);
      return new GroupProxy(new_group_proxy, group_obj.file_id);
    }
    else if (dumb_obj?.type === "Dataset") {
      const dset_obj = dumb_obj as Dataset;
      return new remote.Dataset(dset_obj.file_id, dset_obj.path);
    }
    else if (dumb_obj?.type === "Datatype") {
      return new remote.Datatype();
    }
    else if (dumb_obj?.type === "BrokenSoftLink") {
      const link_obj = dumb_obj as BrokenSoftLink;
      return new remote.BrokenSoftLink(link_obj?.target);
    }
    return 
  }
}
...
export async function save_to_memfs(file: File) {
  const { name } = file;
  console.log(`Saving file ${name} to memfs...`);
